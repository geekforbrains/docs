...
The full system prompt is assembled from multiple sources, ordered from most stable (cache-friendly) to least:

1. **`SYSTEM.md`** — system preamble (auto-created at `~/.operator/SYSTEM.md`)
2. **`AGENT.md`** — agent prompt body (verbatim)
3. **Context block** — platform, channel, user, workspace info (or job block for jobs)
4. **Pinned memories** — always injected regardless of similarity (chat only)
5. **Available skills** — discovered from `skills/*/SKILL.md`
