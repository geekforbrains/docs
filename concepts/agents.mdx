...
- `allow` and `deny` are mutually exclusive per category.
- `allow` — only listed names are available; everything else is hidden.
- `deny` — everything is available except listed names.
- Denied tools and skills are removed from the LLM's view entirely.
