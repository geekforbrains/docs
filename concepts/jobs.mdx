...
Set `model` in the frontmatter to override the agent's configured model for this job. Uses [LiteLLM format](https://docs.litellm.ai/docs/providers) (`provider/model-name`). When omitted, the agent's model chain from `operator.yaml` is used.
